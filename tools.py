...
``slm-data/job_description.json``, the exact task definition the SLM was trained on,
so the demo and the model never drift apart.
...
JOB_DESCRIPTION_PATH = Path(__file__).parent / "slm-data" / "job_description.json"
